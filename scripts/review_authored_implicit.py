#!/usr/bin/env python3
"""Interactive ACCEPT/REJECT review for authored MIXED_IMPLICIT candidates.

Does not modify the candidate JSONL or training_data.json. Decisions are saved
to ``dataset/planner/stage_a_authored_implicit_reviews.jsonl``.
"""
...
def run_summary(candidates_path: Path, reviews_path: Path) -> int:
    ...
    validate_authored_candidate_set(candidates)
    reviews = load_authored_reviews(reviews_path)
    print(format_authored_summary(summarize_authored_reviews(candidates, reviews)))
    return 0


def _print_prompt(session: AuthoredImplicitReviewSession) -> None:
    candidate = session.current()
    if candidate is None:
        print("All authored candidates have been reviewed.")
        print(format_authored_summary(session.summary()))
        return
    position, total = session.current_position()
    print("-" * 50)
    print(f"Authored implicit candidate {position} / {total}")
    ...
    try:
        while True:
            ...
            try:
                ...
                return 0
            try:
                action, status = parse_authored_review_command(raw)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--summary",
        action="store_true",
        help="print progress without interactive review",
    )
